define(function (require, exports, module) {
    ...
    require("zepto/data");
    require("plugin/maskLayer");

    var youkuApi = require("video/jsapi"),
        page = require('page'),
        $videoPages = $(".page-video");

    !function () {
            var playerIndex = 0, YoukuVideo = function ($item, options) {
                console.log( "YoukuVideoIndex"+$item);
            var theClass = this;
            ...
            this.settings = $.extend({
                ...
                url: videoUrl && videoUrl.indexOf("youku") >= 0 ? videoUrl : "http://v.youku.com/v_show/id_XNzU1MTM3MDAw.html",
                ...
                autoplay: !0,
                events: {
                    onPlayerReady: theClass.settings.onPlayerReady, onPlayStart: function (e) {
                        theClass._isPlayStart = !0, theClass.settings.onPlayStart(e)
                    }, onPlayEnd: theClass.settings.onPlayEnd
                }
            }), this._isPlayStart = !1, this.$target.on(!Modernizr.touch ? "click" : "tap", function () {
    ...
    module.exports = {
        init: function () {
            $videoPages.each(function (i, item) {
                ...
                youkuVideoLayer =function(obj){
                    $(obj).find(".m-youkuVideoLayer").maskLayer({
                        onShow: function () {
                            $btnPlay.hide();
                            ...
                            youkuVideo = $(obj).find(".m-youkuVideo").youkuVideo().youkuVideo("getPluginObject");
                        }, onHide: function () {
                            $btnPlay.show();
                            page.enableFlipPage();
                            if (gVar.bkMusicIsOpen == 1) {
                    ...
                    }).maskLayer("getPluginObject").show();

                }

                $btnPlay.on(!Modernizr.touch ? "click" : "tap", function () {
                   // console.log($(this));
                    $page = $(this).parent().parent();
                    //console.log($page);
                    youkuVideoLayer($page);
